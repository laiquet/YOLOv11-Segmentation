...
from utils import (smart_inference_mode, select_device,
                    check_img_size, colorstr, Profile, strip_optimizer, sorting_custom,
                    plot_lineplot, plot_swarmplot, plot_boxplot, plot_scatterplot, plot_violinplot)
from helpers import (process_mask, non_max_suppression, masks2segments, scale_coords,
                     increment_path, colors, plot_masks, scale_masks, save_one_box, scale_boxes, xywh2xyxy, scale_masks_v11)
    ...
    for path, im, im0s, s in dataset:
        ...
        for i, det in enumerate(preds):  # per image
            ...
            if len(det):
                ...
                masks = process_mask(protos[i], det[:, 6:], det[:, :4], im.shape[2:], im0.shape)  # HWC
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    if save_txt:  # Write to file
                        ...
                        if len(pixel_vertices)>2:
                            ...
                            # Vectorized binning using np.digitize (replaces per-pixel Python loop)
                            pixel_vals = seg_spot[xs, ys]
                            bin_edges = np.arange(0.0, 1.1, 0.1)  # [0.0, 0.1, 0.2, ..., 1.0]
                            bin_indices = np.digitize(pixel_vals, bin_edges, right=True)  # bins 1-10
                            bin_indices = np.clip(bin_indices, 1, 10)  # ensure range [1, 10]
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT+'/train-seg/weights/best_yolov8_modified_spots.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT+'../../Meeting_13_03_25/New_Meeting_Data2/Inference data/4. Segmentations/resized/', help='file/dir')
